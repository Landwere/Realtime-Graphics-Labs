...
void RotateMatrix(Eigen::Matrix3f& matrix, float r_x, float r_y, float r_z)
{
	matrix =
		Eigen::AngleAxisf((r_x / 180) * M_PI, Eigen::Vector3f::UnitX()) *
		Eigen::AngleAxisf((r_y / 180) * M_PI, Eigen::Vector3f::UnitY()) *
		Eigen::AngleAxisf((r_z / 180) * M_PI, Eigen::Vector3f::UnitZ());
}

void SheerMatrix(Eigen::Matrix3f& matrix, float sheerAmount)
{
	Eigen::Matrix3f sheerMatrix;
	sheerMatrix <<
		sheerAmount, sheerAmount, 0,
		0, sheerAmount, 0,
		0, 0, sheerAmount;
...
int main()
{
...

	{
		...
		//transformationMatrix.x = Eigen::AngleAxisf(45, Eigen::Vector3f::UnitX());
